	...
	devtools::load_all(path = lime_path)
 	 ...
 	 lyears <- as.numeric(str_split(as.character(scen_grid[x,"L"]),"L")[[1]][2])
 	 	...
 	 	if(file.exists(file.path(em_path, "LIME_output.rds")) == FALSE){
			...
			first_rdev <- max(first_yr - agemax,1)

			est_rdev_t_inp <- rep(1,length(years))
			if(first_rdev > 1) est_rdev_t_inp[1:(first_rdev-1)] <- 0

			if(scen_grid[x,"Data"] == "LC"){
				data_avail <- "LC"
				C_type <- 0
				SigRpen <- 1
				Fpen <- 1
			}
			if(scen_grid[x,"Data"] == "Catch_LC"){
				data_avail <- "Catch_LC"
				C_type <- 2
				SigRpen <- 0
				Fpen <- 0
...
re <- lapply(1:nrow(scen_grid), function(x){
	byIter <- lapply(1:length(itervec), function(y){
		res1 <- readRDS(file.path(lime_dir, "short_slow", "F1", "LowSigmaR", "N200", 1, "L20", "LC", "LIME_output.rds"))
		res2 <- readRDS(file.path(lime_dir, "short_slow", "F1", "LowSigmaR", "N200", 1, "L75", "LC", "LIME_output.rds"))
		t1 <- true %>% filter(lifehistory == "short_slow") %>% filter(iteration == 1) %>% filter(Fscen == "F1") %>% filter(SigmaR == "LowSigmaR")
