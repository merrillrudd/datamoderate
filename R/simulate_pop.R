simulate_pop <- function(df, path, itervec, f_list, lh_df, ncores = 1, rewrite){
  ...
  for(x in 1:nrow(df)){
    ...
    run_iters <- foreach(i = 1:length(itervec), .packages = c('ss3sim', 'r4ss')) %dopar%{
      ...
      fmsy <- as.numeric(lh_info %>% filter(ShortName == lh) %>% dplyr::select(Fmsy))
      if(df[x,"F"] == "F1") fvec <- f_list[["F1"]]
      if(df[x,"F"] == "F2") fvec <- f_list[["F2"]]
      if(df[x,"F"] == "F3") fvec <- f_list[["F3"]]
      f1 <- fvec * fmsy
      change_f(years = 1:length(f1), fisheries = 1, fvals = f1, seasons = 1, ses = 0.01, ctl_file_in = file.path(om_path, paste0(lh, "OM.ctl")), ctl_file_out = file.path(om_path, paste0(lh, "OM.ctl")))
