      ...
      byIter <-  foreach(y = 1:length(itervec), .packages = c("tidyverse","r4ss")) %dopar%{
        ...
        # for(z in 1:length(Rscen)){
          ...
          if(file.exists(file.path(res_dir, Rscen[z], "Report.sso"))){
            r1 <- tryCatch(r4ss::SS_output(file.path(res_dir, Rscen[z]), verbose = FALSE, printstats = FALSE, hidewarn = TRUE), error = function(e) NA)
            # rx <- r4ss::SS_output(file.path(res_dir, "perfect", "L100", "R_biasadjusted"))
            if(all(is.na(r1)) == FALSE){
              d1 <- get_results_derived(r1)
              # dx <- get_results_derived(rx)
