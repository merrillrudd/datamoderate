run_ss <- function(df, 
  ...
  for(x in 1:nrow(df)){
    ...
    run_iters <- foreach(i = 1:length(itervec), .packages = c('ss3sim', 'r4ss')) %dopar%{
      ...
      perfect <- SS_splitdat2(inpath = om_path, outpath = samp_path, inname = "data.ss_new", outpattern = "perfect", number = FALSE, verbose = TRUE, fillblank = TRUE, MLE = TRUE)
    
      dat <- SS_readdat(file.path(samp_path, "perfect.ss"), version = "3.30", verbose = FALSE)
      # save_catch <- dat$catch
      dat$catch$catch_se[which(dat$catch$catch_se > 0)] <- 0.01
      if(df[x,"Rich"] == TRUE){
      ...
      if(df[x,"Samp"] %in% c("Ndecline") == FALSE){
        set.seed(456)
        for(y in 1:Nyears){
          ...
          if(sum(year_temp)!=0){
            ...
            if(df[x,"Samp"] != "perfect") Nsamp <- as.numeric(strsplit(as.character(df[x,"Samp"]),"N")[[1]][2])
            if(df[x,"Samp"] == "perfect") Nsamp <- 1000
      ...
      if(file.exists(file.path(rpath1, "Report.sso")) == FALSE | rewrite == TRUE){
        ...
        ctl$size_selex_parms[1,"HI"] <- 120.5
        ctl$size_selex_parms[7,"LO"] <- 11.5
        ctl$size_selex_parms[7,"HI"] <- 120.5
        
        df$Fix <- as.character(df$Fix)
        df$Adjust <- as.character(df$Adjust)
        ...
        # plot(d2$Value.Bratio[which(d2$Yr > 0)], type = "l")
        # lines(d1$Value.Bratio[which(d2$Yr > 0)], col = "blue")

        # plot(d2$Value.F[which(d2$Yr > 0)], type = "l")
        # lines(d1$Value.F[which(d2$Yr > 0)], col = "blue")
      ...
      if(file.exists(file.path(rpath2, "Report.sso")) == FALSE | rewrite == TRUE){
        ...
        rep_bias <- r4ss::SS_output(rpath1, covar = TRUE, printstats = FALSE, forecast = FALSE, verbose = FALSE)
        ...
        if(all(is.na(new_bias))==FALSE){
          ...
          # lines(d1$Value.Recr[which(d1$Yr %in% 1:100)], col = "blue")
          # lines(d3$Value.Recr[which(d3$Yr %in% 1:100)], col = "red")
          # plot(d2$Value.Bratio[which(d2$Yr %in% 1:100)], type = "l")
          # lines(d1$Value.Bratio[which(d1$Yr %in% 1:100)], col = "blue")
          # lines(d3$Value.Bratio[which(d3$Yr %in% 1:100)], col = "red")
